...
def evaluate_piqa(model, tokenizer, dataset, args, device):
    total_correct = 0
    total_questions = 0
    results = []
    ...
    for i in tqdm(range(num_batches), desc="Evaluating PIQA", unit="batch"):
        ...
        questions = [item["goal"] for item in batch_data]
        choices_A = [item["sol1"] for item in batch_data]
        choices_B = [item["sol2"] for item in batch_data]
        ground_truths = ["A" if item["label"] == 0 else "B" for item in batch_data]

        correct = 0
        for q, a, b, gt in zip(questions, choices_A, choices_B, ground_truths):
            prompt = f"Question: {q}\nAnswer: "

            prompt_ids = tokenizer(prompt)["input_ids"]
            answer_A_ids = tokenizer(a)["input_ids"]
            answer_B_ids = tokenizer(b)["input_ids"]

            prompt_tensor = torch.tensor(prompt_ids).to(device)
            answer_A_tensor = torch.tensor(answer_A_ids).to(device)
            answer_B_tensor = torch.tensor(answer_B_ids).to(device)

            log_likelihood_A = get_log_likelihood(model, prompt_tensor, answer_A_tensor, mc_num=args.mc_num, cfg_scale=args.cfg_scale)
            log_likelihood_B = get_log_likelihood(model, prompt_tensor, answer_B_tensor, mc_num=args.mc_num, cfg_scale=args.cfg_scale)

            predicted_label = "A" if log_likelihood_A > log_likelihood_B else "B"
            ...
            results.append({
                ...
                "choice_A": a,
                "choice_B": b,
                "log_likelihood_A": log_likelihood_A,
                "log_likelihood_B": log_likelihood_B,
                "predicted_label": predicted_label,
                "ground_truth": gt,
                "correct": predicted_label == gt
            })

        total_correct += correct
        total_questions += len(batch_data)

        print(f"Batch {i+1}/{num_batches}: Accuracy {correct}/{len(batch_data)}")

    final_acc = total_correct / total_questions
    print(f"Final Accuracy for PIQA: {final_acc:.4f}")
    save_results("piqa", results, final_acc, args.model)

def evaluate_gsm8k(model, tokenizer, dataset, args, device):   
...
def evaluate_math(model, tokenizer, dataset, args, device):   
    start_time = time.time()
    ...
    system_prompt = (
        "prefix:  Only reply with the correct answer."
    )
...
def evaluate_humaneval(model, tokenizer, args, device):
...
def evaluate_mt(model, tokenizer, dataset, args, device):
    """
    Batched evaluation of LLaDA on English-to-French translation (WMT14 dataset).
    """
    start_time = time.time()
    ...
    for i in tqdm(range(num_batches), desc="Evaluating MT (fr→en)", unit="batch"):
        batch_start_time = time.time()
        ...
        for sample, generated_text in zip(batch_data, generated_texts):
            ...
            ground_truth = sample["translation"]["fr"]
        ...
        print(f"[Batch {i+1}/{num_batches}] Avg BLEU: {batch_bleu / batch_size:.4f}, Avg METEOR: {batch_meteor / batch_size:.4f} | Batch Time: {batch_time:.2f} sec")
...
def evaluate_sudoku(model, tokenizer, args, device):
...
def evaluate():
    ...
    if args.task == "gsm8k":
        ...
        evaluate_gsm8k(model, tokenizer, dataset, args, device) 
    elif args.task == "math":
        dataset = load_dataset("di-zhang-fdu/MATH500", split="test")
        evaluate_math(model, tokenizer, dataset, args, device) 
    elif args.task == "piqa":
        dataset = load_dataset("ybisk/piqa", split="validation") 
        evaluate_piqa(model, tokenizer, dataset, args, device) 
    elif args.task == "humaneval":
        evaluate_humaneval_ar(model, tokenizer, args, device)
    elif args.task == "mt":
        dataset = load_dataset("wmt14", "fr-en", split="test")
        evaluate_mt(model, tokenizer, dataset, args, device)
    elif args.task == "sudoku":
        evaluate_sudoku(model, tokenizer, args, device)
    else:
