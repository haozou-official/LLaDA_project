...
def forward_process(batch, prompt_index, mask_id):
    ...
    expected_length = batch.shape[1]  
    if is_mask.shape[1] < expected_length:
        pad_size = expected_length - is_mask.shape[1]
        is_mask = torch.cat((is_mask, torch.zeros(batch.shape[0], pad_size, dtype=torch.bool, device=batch.device)), dim=1)
    elif is_mask.shape[1] > expected_length:
        is_mask = is_mask[:, :expected_length]  
